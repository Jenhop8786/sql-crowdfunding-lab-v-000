...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS remaining FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.id HAVING remaining >= 0;"
end
